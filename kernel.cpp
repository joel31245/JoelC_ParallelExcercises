...
#endif
 /* Compute pi using midpoint-rule integration with varying numbers of points.*/
  for (itrial=1; itrial<=4; itrial++){
      n = powl(10, itrial+5); /* Number of iterations, 10^(itrial+5) */
      deltax = 2. / (double) n;      /* Integration grid spacing */
      gettimeofday (&tstart, NULL);  /* Start the timer */  
      /*compute x values */
      /*!!!! (1) Add the OpenMP statement here; (2)  What needs to be private? */
	...
	x = -1. + i*deltax;
      ...
      dt = (double)(tstop.tv_sec - tstart.tv_sec) 
