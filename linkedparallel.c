...
int main(int argc, char *argv[]) {
     ...
     struct node *head=NULL;
	 struct node *p;
     int threads=2;   //assume 2 threads by default


     printf("Process linked list\n");
     printf("  Each node will compute %d iterations of a numeric computation\n",N);
     printf("  of a function called process_work.\n");
     printf("  process_work(n+1) takes more time to compute that process_work(n)\n");
     printf("  for this calculation - process_work starts computing at process_work(%d)\n",
FS);

     if (argc >1) {
        threads = atoi(argv[1]); // 1st input parameter: number of threads
     }     
     ...
     q = head;  //print the resulting computations as a validation check
     while (q != NULL) {
        printf("%d : %d\n",q->data, q->fibdata);
        temp = q->next;
        free (q);
